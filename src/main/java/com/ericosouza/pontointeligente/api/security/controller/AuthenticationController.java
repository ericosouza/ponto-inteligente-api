package com.ericosouza.pontointeligente.api.security.controller;
...
import com.ericosouza.pontointeligente.api.security.dto.JwtAuthenticationDto;
import com.ericosouza.pontointeligente.api.security.dto.TokenDto;
import com.ericosouza.pontointeligente.api.security.utils.JwtTokenUtil;
...
public class AuthenticationController {
	...
	public ResponseEntity<Response<TokenDto>> gerarTokenJwt(@Valid @RequestBody JwtAuthenticationDto authenticationDto, BindingResult result) throws AuthenticationException {
		...
		if (result.hasErrors()) {
			AuthenticationController.log.error("Erro validando lançamento: {}", result.getAllErrors());
			result.getAllErrors().forEach(error -> response.getErrors().add(error.getDefaultMessage()));
			return ResponseEntity.badRequest().body(response);
		}

		AuthenticationController.log.info("Gerando token para o email {}.", authenticationDto.getEmail());
	...
	public ResponseEntity<Response<TokenDto>> gerarRefreshTokenJwt(HttpServletRequest request) {
		AuthenticationController.log.info("Gerando refresh token JWT.");
		Response<TokenDto> response = new Response<TokenDto>();
