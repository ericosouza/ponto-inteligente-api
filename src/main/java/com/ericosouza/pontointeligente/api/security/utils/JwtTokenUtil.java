package com.ericosouza.pontointeligente.api.security.utils;
...
public class JwtTokenUtil {
	...
	public String obterToken(UserDetails userDetails) {
		...
		claims.put(JwtTokenUtil.CLAIM_KEY_USERNAME, userDetails.getUsername());
		userDetails.getAuthorities().forEach(authority -> claims.put(JwtTokenUtil.CLAIM_KEY_ROLE, authority.getAuthority()));
		claims.put(JwtTokenUtil.CLAIM_KEY_CREATED, new Date());
	...
	private String gerarToken(Map<String, Object> claims) {
		return Jwts.builder().setClaims(claims).setExpiration(this.gerarDataExpiracao()).signWith(SignatureAlgorithm.HS512, this.secret).compact();
	}
